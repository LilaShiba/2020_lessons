...
def rr(arr):
    total = sum(arr)
    if total % 2 == 0:
        return canpp(arr,total,0,0,{})
    return False

def canpp(arr, total, idx, cur_sum, memo):
    if (idx, cur_sum) in memo:
        return memo[(idx,cur_sum)]
    if cur_sum * 2 == total:
        return True 
    if cur_sum * 2 > total or idx >= len(arr):
        return False 
    
    memo[(idx, cur_sum)] = canpp(arr, total, idx+1, cur_sum+arr[idx], memo) or canpp(arr, total, idx+1, cur_sum, memo)
    return memo[(idx, cur_sum)]
